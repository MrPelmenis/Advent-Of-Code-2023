...
function start(){
    ...
    console.log(data);
    let sCoords = findSCoordinates();
    let neighbours = getMyNeighbours(sCoords.i,sCoords.j);
    neighbours = neighbours.filter(neighbour =>{
        return neighbourWalkthrough(neighbour).canPass;
    });

    let nextMoves = neighbourWalkthrough(neighbours[0]).cellAfterMeDir;

    vertices.push([sCoords.i, sCoords.j]);

    goToTheNextCell(sCoords.i + neighbours[0].dir.y, sCoords.j + neighbours[0].dir.x, nextMoves.x, nextMoves.y);

}


function goToTheNextCell(myI, myJ, myMoveX, myMoveY){
    steps++;
    let nextCellSymbol = data[myI+myMoveY][myJ+myMoveX];

    vertices.push([myI, myJ]);

    if(nextCellSymbol == "S"){
        console.log("steps: " + ((steps + 1)/2));
        let area = calculatePolygonArea(vertices);
    }else{
        let nextMoves = neighbourWalkthrough({symbol:nextCellSymbol, dir:{x:myMoveX, y:myMoveY}});

        setTimeout(() => {
            console.log(nextCellSymbol);
            goToTheNextCell(myI+myMoveY,myJ+myMoveX,nextMoves.cellAfterMeDir.x,nextMoves.cellAfterMeDir.y);    
        }, 1);
...
function calculatePolygonArea(){
    ...
    for(let i=0;i<data.length;i++){
        for(let j=0;j<data[i].length;j++){
            ...
            if(isPointInsidePolygon([i,j], vertices)){
               if(!isPointInVerticiesArray([i,j])){
                    area++;
                }
            }
...
function isPointInVerticiesArray(point){
    let res = false;
    vertices.forEach( verticie =>{
        if(verticie[0] == point [0] && verticie [1] == point[1]){
            res = true; 
            return res;
        };
    })
...
function neighbourWalkthrough(neighbour){
    let answer = {canPass:false, cellAfterMeDir:{y:0,x:0}};
    switch (neighbour.symbol){
        case "|": {
            if(neighbour.dir.x == 0){
                if((neighbour.dir.y == 1)){
                    answer = {canPass:true, cellAfterMeDir:{x:0,y:1}, symbol:"|"};
                }
                if((neighbour.dir.y == -1)){
                    answer = {canPass:true, cellAfterMeDir:{x:0,y:-1}, symbol:"|"};
                }
        ...
        case "-": {
            if (neighbour.dir.y == 0) {
                if (neighbour.dir.x == 1) {
                    answer = { canPass: true, cellAfterMeDir: { x: 1, y: 0 } , symbol:"-"};
                } else if (neighbour.dir.x == -1) {
                    answer = { canPass: true, cellAfterMeDir: { x: -1, y: 0 } , symbol:"-"};
                }
            }
            break;
        ...
        case "L": {
            if (neighbour.dir.x == 0 && neighbour.dir.y == 1) {
                answer = { canPass: true, cellAfterMeDir: { x: 1, y: 0 } , symbol:"L"};
            } else if (neighbour.dir.x == -1 && neighbour.dir.y == 0) {
                answer = { canPass: true, cellAfterMeDir: { x: 0, y: -1 } , symbol:"L"};
            }
            break;
        }
    
        case "7": {
            if (neighbour.dir.x == 1 && neighbour.dir.y == 0) {
                answer = { canPass: true, cellAfterMeDir: { x: 0, y: 1 }, symbol:"7" };
            } else if (neighbour.dir.x == 0 && neighbour.dir.y == -1) {
                answer = { canPass: true, cellAfterMeDir: { x: -1, y: 0 } , symbol:"7"};
            }
            break;
        }
        ...
        case "J": {
            if (neighbour.dir.x == 1 && neighbour.dir.y == 0) {
                answer = { canPass: true, cellAfterMeDir: { x: 0, y: -1 }, symbol:"J" };
            } else if (neighbour.dir.x == 0 && neighbour.dir.y == 1) {
                answer = { canPass: true, cellAfterMeDir: { x: -1, y: 0 }, symbol:"J" };
            }
            break;
        }
        ...
        case "F": {
            if (neighbour.dir.x == 0 && neighbour.dir.y == -1) {
                answer = { canPass: true, cellAfterMeDir: { x: 1, y: 0 }, symbol:"F" };
            } else if (neighbour.dir.x == -1 && neighbour.dir.y == 0) {
                answer = { canPass: true, cellAfterMeDir: { x: 0, y: 1 }, symbol:"F" };
            }
            break;
        }
    }
...
function getMyNeighbours(i,j){
    let neighbours = [];
    if(i>0 && data[i-1][j]!="."){
        neighbours.push({symbol: data[i-1][j], dir:{y:-1,x:0}});
    }

    if(i<data[0].length - 1 && data[i+1][j]!="."){
        neighbours.push({symbol: data[i+1][j], dir:{y:+1,x:0}});
    }


    if(j>0 && data[i][j-1]!="."){
        neighbours.push({symbol: data[i][j-1], dir:{y:0,x:-1}});
    }

    if(j<data.length - 1 && data[i][j+1]!="."){
        neighbours.push({symbol: data[i][j+1], dir:{y:0,x:1}});
    }
