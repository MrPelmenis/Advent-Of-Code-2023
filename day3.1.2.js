...
function start(){
    data = strings.split("\n");
    data = data.map(dat => dat.split(""));
    tableW = data[0].length;
    tableH = data.length;

    let string = strings.replaceAll("\n","");

    for(let i=0;i< data.length;i++){
        for(let j=0;j<data[i].length;j++){
            if(data[i][j] == "*"){
                let l = getNumberToTheLeft(i,j);
                let r = getNumberToTheRight(i,j);
                let u = getNumberMultiplicationOnTop(i,j);
                let u1 = 0;
                let u2 = 0;
                let d1 = 0;
                let d2 = 0;
                let d = getNumberMultiplicationDown(i,j);
                ...
                if(multiplication != 1 && only2(r,l,u1,u2,d1,d2)){
...
function only2(r,l,u1,u2,d1,d2){
    const notOneCount = [r, l, u1, u2, d1, d2].filter(num => num != 0).length;
    return notOneCount == 2;
}



function getNumberMultiplicationOnTop(i,j){
    if(i > 0){
        let currentNumber = "";
        let isCurrentNumAdjacent = false;
        let topNumbers =[];
        for(let r = 0;r < data[i-1].length;r++){
            let currentChar = data[i-1][r];
            if(numbers.indexOf(currentChar) != -1){
                if(!isCurrentNumAdjacent){
                    isCurrentNumAdjacent = areCellsAdjacent(i-1,r, i,j);
                }
                ...
                if(isCurrentNumAdjacent){
                    isCurrentNumAdjacent = false;
                    topNumbers.push(JSON.parse(currentNumber));
                }else{
                    if(currentNumber != ""){
                    }
...
function getNumberMultiplicationDown(i,j){
    if(i < data.length-1){
        let currentNumber = "";
        let isCurrentNumAdjacent = false;
        let topNumbers =[];
...
function areCellsAdjacent(i1, j1, i2, j2) {
    const iDiff = Math.abs(i1 - i2);
    const jDiff = Math.abs(j1 - j2);

    return (iDiff == 1 && jDiff == 0) || (jDiff == 1 && iDiff == 0) || (iDiff == 1 && jDiff == 1);
}




function getNumberToTheLeft(i,j){
    let num = "";
    for(let k = j-1;k>= 0;k--){
        if(numbers.indexOf(data[i][k]) != -1){
            num = data[i][k] + num;
        }else{
            break;
        }
    }
    return num ==""? 0 : JSON.parse(num);
}

function getNumberToTheRight(i,j){
    let num = "";
