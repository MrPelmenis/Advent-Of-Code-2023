let strings = `Game 1: 5 red, 1 green; 6 red, 3 blue; 9 red; 1 blue, 1 green, 4 red; 1 green, 2 blue; 2 blue, 1 red
...
function start(){
    ...
    totalSum = 0;
    arr.forEach(inputStr =>{
        let isPossible = true;
        let id = inputStr.substring(5, inputStr.indexOf(":"));
        let redArr = (inputStr.split("red"));

        let minRed = 0;
        let minBlue = 0;
        let minGreen = 0;

        redArr.forEach(redPart=>{
            if(redPart[redPart.length - 1] == " "){
                //ir sarkans
                let redcount = JSON.parse(redPart.substring(redPart.length-3, redPart.length-1));
                minRed = Math.max(minRed, redcount);
            }
        });
        if(isPossible){
            let blueArr = (inputStr.split("blue"));
            //console.log(blueArr);
            blueArr.forEach(redPart=>{
                if(redPart[redPart.length - 1] == " "){
                    //ir zils
                    let redcount = JSON.parse(redPart.substring(redPart.length-3, redPart.length-1));
                    minBlue = Math.max(minBlue, redcount);
                }
            });
        }

        if(isPossible){
            let greenArr = (inputStr.split("green"));
            greenArr.forEach(redPart=>{
                if(redPart[redPart.length - 1] == " "){
                    //ir zals
                    let redcount = JSON.parse(redPart.substring(redPart.length-3, redPart.length-1));
                    minGreen = Math.max(minGreen, redcount);
                }
            });
        }

        console.log(id + " " + (minRed*minBlue*minGreen));
