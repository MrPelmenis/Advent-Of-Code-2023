...
function start(){
    ...
    arr.forEach(inputStrR =>{
        let integers = ["1","2","3","4","5","6","7","8","9","0"];
        let inputStr = inputStrR;
        let integerWords = ["zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine"];

        for(let i =0;i<integerWords.length;i++){
            let currentWord = integerWords[i]
            inputStr = inputStr.replaceAll(currentWord, currentWord[0] + i + currentWord[currentWord.length-1]);
        }
        ...
        for(let i = 0;i<inputStr.length;i++){
            if(integers.indexOf(inputStr[i]) != -1){
                firstNum = (inputStr[i]);
                break;
            }
        }
        for(let i = inputStr.length;i>=0;i--){
            if(integers.indexOf(inputStr[i])!=-1){
                secondNum = (inputStr[i]);
                break;
            }
        }
        
        totalSum += JSON.parse(firstNum+secondNum);
    })
